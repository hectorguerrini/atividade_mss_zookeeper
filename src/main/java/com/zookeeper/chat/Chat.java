...
public class Chat {
    ...
    private void capturaUsuario () throws InterruptedException, KeeperException{
        ...
        String input;
        do {
            System.out.println("Escolha um nome de usuario para voce.");
            input = scanner.nextLine();
            nameValid = usuarioJaExiste(input) && !input.contains("/");
        } while (nameValid);
        usuario = input;
        String zNodeUser = zooKeeper.create(String.format("%s/%s",ZNODE_USUARIOS,usuario), new byte[]{}, ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL_SEQUENTIAL);
    ...
    private void exibirHistorico () throws InterruptedException, KeeperException{
        ...
        for (String data : datas){
            ...
            String txt = msg.substring(msg.indexOf(":")+1,msg.length());       
            System.out.printf("%s: %s diz %s\n", formatDate(Long.valueOf(data)),user, txt);
            System.out.println("************************");
        }
    }
    ...
    private void executar() throws InterruptedException, KeeperException{
        ...
        while (!opcao.equals("/exit")){
            ...
            //send
                ...
                String path = String.format("%s/%d", ZNODE_CHAT, dateNumber);
                String msg = opcao.substring(opcao.indexOf(" ") + 1);
                String znodeChat = zooKeeper.create(path, String.format("%s:%s",usuario,msg).getBytes(StandardCharsets.UTF_8),ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
            }
            else{
